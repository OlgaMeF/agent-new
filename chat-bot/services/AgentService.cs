...
/// <summary>
/// Conversational agent for the learn-skills platform.
/// The request flows through six phases: perception, reasoning, planning,
/// execution, response synthesis and state persistence. Every fact shown to the
/// user originates from an MCP tool result; the language model only writes prose
...
public class AgentService
{
    ...
    private const int MaxCardsForCourseLists = 8;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            return "Bitte stell mir eine Frage – zum Beispiel zu Kursen, Skills, Profilen oder Lernpfaden.";
        }

        if (message.Length > MaxMessageLength)
        {
            return $"Bitte kürze deine Frage auf höchstens {MaxMessageLength} Zeichen.";
        }
        ...
        try
        {
            ...
            conversation = GetConversationState(perception.ConversationId);
            await conversation.ProcessingGate.WaitAsync(cancellationToken);
            conversationGateHeld = true;

            _logger.LogInformation(
                "PHASE1_PERCEPTION conversation={ConversationId}, messageLength={MessageLength}",
                perception.ConversationId,
                perception.OriginalMessage.Length);

            // ---------- PHASE 2: REASONING ----------
            var reasoning = await AnalyzeMessageAsync(perception, conversation, cancellationToken);
            reasoning = ResolveToolCalls(reasoning, conversation);
            reasoning = ValidateReasoning(reasoning, conversation);

            _logger.LogInformation(
                "PHASE2_REASONING intent={Intent}, language={Language}, toolCalls={ToolCalls}",
                reasoning.Intent,
                reasoning.Language,
                DescribeToolCalls(reasoning.ToolCalls));

            // ---------- PHASE 3: PLANNING ----------
            var plan = CreatePlan(reasoning, conversation, perception.OriginalMessage);

            _logger.LogInformation(
                "PHASE3_PLAN needsData={NeedsData}, round1={Round1}",
                plan.NeedsData,
                DescribeToolCalls(plan.FirstRound));

            // ---------- PHASE 4: EXECUTION ----------
            var evidence = await ExecutePlanAsync(plan, conversation, cancellationToken);

            _logger.LogInformation(
                "PHASE4_EXECUTION tools={Tools}, courses={Courses}, profiles={Profiles}, skills={Skills}",
                string.Join(",", evidence.ExecutedTools),
                evidence.Courses.Count,
                evidence.Profiles.Count,
                evidence.Skills.Count);

            // ---------- PHASE 5: RESPONSE ----------
            var answer = await ComposeAnswerAsync(plan, evidence, conversation, cancellationToken);

            _logger.LogInformation(
                "PHASE5_RESPONSE length={Length}, cards={Cards}",
            ...
            _logger.LogInformation(
                "PHASE6_STATE courses={Courses}, profiles={Profiles}, skills={Skills}, pendingSlot={Slot}",
                conversation.LastCourses.Count,
                conversation.LastProfiles.Count,
                conversation.LastSkills.Count,
    ...
    {
        var prompt = BuildReasoningPrompt(request, conversation);

        var aiAnswer = await _genAiService.GetChatCompletionAsync(prompt, cancellationToken);

        _logger.LogInformation("PHASE2_REASONING_RESPONSE received={Received}", !string.IsNullOrWhiteSpace(aiAnswer));

        if (string.IsNullOrWhiteSpace(aiAnswer)
            || !TryExtractFirstJsonObject(aiAnswer, out var json)
            || !TryParseReasoning(json, out var reasoning))
        {
            // The classifier is unavailable or answered off-format. A plain course
            // search on the raw message is a better default than giving up.
        ...
        ConversationState conversation)
    {
        ...
        return $$"""
...

            {
              ...
              "language": "<ISO code of the user's language, e.g. de or en>",
              "clarificationQuestion": "<question in the user's language, only for intent=clarify>",
              "slots": { "currentProfile": "", "targetProfile": "", "division": "", "topic": "" },
              "toolCalls": [
                  { "tool": "<tool name>", "query": "<argument or empty>", "ref": "none|active|first|second|third|fourth|fifth" }
              ]
            }
            ...
            - course_compare          Compare two or more courses, or "which one is better / best for beginners".
            - skill_search            Which skills or skill categories exist in general, or the
                                      whole hierarchy. Only for an overview without a named skill.
            - skill_details           One named skill: explain it, define it, or list its
                                      sub-skills. Use this for "What is X", "Explain X",
                                      "What does X mean", "Which skills are under X".
            - skill_courses           Which courses train a given skill.
            - profile_search          Find learning profiles / job roles.
            - profile_details         Details and skills of one profile or job role, including
                                      "which skills / competencies does role X need".
            - profile_courses         The courses belonging to one profile (required vs optional).
            - learning_path           A path from a current role to a target role.
            - learning_recommendation Wants recommendations but named no topic, role or goal yet.
            - division_overview       Which divisions/areas/departments exist, or content of one area.
            - collections             Curated collections or topic packages.
            - bookmarks               The user's own saved courses and profiles.
            - progress                The authenticated user's completed and in-progress courses
                                      and the site-wide completion rate.
            ...
            - clarify                 The request is too vague to search; ask one precise question.
            ...
            - "ref" points into the conversation context instead of a literal query.
              Use "active" for "this course / that profile", and "first" through "fifth"
              for positional references such as "the second one". Leave "query" empty then.
            - Every tool call carries its own "ref", so a comparison of two remembered items
              produces two tool calls with ref "first" and "second".
            - Tools marked (no argument) must be called with an empty query.
            - Prefer get_courses_by_tag over search_courses when the user asks which courses
              train a specific skill.
            - Follow-ups such as "weitere Kurse", "ähnliche Kurse", "more courses"
              or "similar courses" are course_search requests when a course was just
              discussed. Do not classify them as clarify; search using the topic in
              the current message and use the remembered course only as context.
            - Use get_profile_skills for "which skills does role X need". Never answer that
              with search_skills — the taxonomy is identical for every role and says
              nothing about the role.
            - Personalization is limited to successful authenticated MCP user-context data.
              Never infer personal skills, goals, profile progress or completed courses from
              the conversation. The progress rate is site-wide, not profile-specific.
            - Use get_skill when the user names one skill, and search_skills only when no
              specific skill is named.
            ...
            - Emit an empty toolCalls array for capabilities, smalltalk, out_of_scope, clarify
              and learning_recommendation.
            - Use intent=clarify only when no sensible search is possible at all.
            - Fill slots only with values the user actually stated.
            - Never invent tool arguments from older assistant answers. Only the
              "Context the backend remembers" block lists cards that still exist.
              Conversation history is for understanding pronouns, not for reusing entities.
        ...
        string userMessage)
    {
        ...
        {
            if (intent == AgentIntent.CourseCompare && conversation.LastCourses.Count >= 2)
            {
                foreach (var course in conversation.LastCourses.Take(3))
                {
                    toolCalls.Add(new ToolCallRequest(
                        "get_course",
    ...
    /// </summary>
    private static SkillInfo? FindNamedSkill(ExecutionPlan plan, Evidence evidence) =>
        evidence.Skills
            .Where(s => s.Name.Length >= 3 && ContainsText(plan.UserMessage, s.Name))
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (plan.Intent is AgentIntent.Clarify or AgentIntent.LearningRecommendation)
        {
        ...
        AppendSuggestions(builder, plan, evidence, language);
        ...
        CancellationToken cancellationToken)
    {
        ...
        var skillInstruction = plan.Intent is AgentIntent.SkillSearch or AgentIntent.SkillDetails
            ? "Explain the requested skill or skill group directly in prose. Include the most relevant names and relationships from DATA; do not ask a follow-up question when DATA contains an answer."
            : string.Empty;
        ...
        var prompt = $$"""
            ...
            Write the answer text for the user request below.
        ...
        var answer = await _genAiService.GetChatCompletionAsync(prompt, cancellationToken);
    ...
    private static (string Rule, int MaxWords, int MaxSentences) ProseBudget(AgentIntent intent) =>
        intent switch
        {
            AgentIntent.CourseCompare or AgentIntent.LearningPath =>
    ...
    private static void AppendSuggestions(
        ...
        string language)
    {
        string[] suggestions = plan.Intent switch
        {
