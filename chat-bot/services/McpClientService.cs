...
public class McpClientService
{
    ...
    /// McpSupportingResources this handler is called get_divisions
    /// </summary>
    ...
    {   
        ...
        var (body, sessionRejected) = await SendToolCallAsync(
            mcpUrl, mcpKey, payloadJson, toolName, cancellationToken);

        if (body is not null || !sessionRejected)
        {
            return body;
        }

        // The cached session id is kept for the lifetime of this client. Once the
        // server has dropped that session every following call would fail with the
        // same 404, so the id is discarded and the call retried on a fresh one.
        _logger.LogInformation("MCP session was rejected; re-initializing and retrying {Tool}.", toolName);
        _sessionId = null;
        ...
        (body, _) = await SendToolCallAsync(mcpUrl, mcpKey, payloadJson, toolName, cancellationToken);

        return body;
    }

    private async Task<(string? Body, bool SessionRejected)> SendToolCallAsync(
        string mcpUrl,
        string mcpKey,
        string payloadJson,
        string toolName,
        CancellationToken cancellationToken)
    {
        ...
        _logger.LogWarning(
            "MCP tool {Tool} failed with status {StatusCode}, sessionRejected={SessionRejected}",
            toolName,
            (int)response.StatusCode,
            sessionRejected);

        return (null, sessionRejected);
    }
    ...
    private string? ResolveMcpUrl()
    {
        ...
        // Cloud Foundry: use the local listener to avoid TLS hostname mismatch.
        var cfPort = Environment.GetEnvironmentVariable("PORT");
        if (!string.IsNullOrWhiteSpace(cfPort))
        {
            return $"http://127.0.0.1:{cfPort}/mcp";
        }
