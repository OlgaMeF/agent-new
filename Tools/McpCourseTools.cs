...
public class McpCourseTools
{
    ...
    /// </summary>
    [McpServerTool, Description("Search published courses on the learn-skills platform. Filter by text query, platform (e.g. 'LinkedIn Learning'), skill level, cluster name, or division. Returns paginated course card summaries with title, instructor, duration, platform, and deep link.")]
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var cacheKey = $"courses:search:{McpCacheService.HashParams(new { query, platform, skillLevel, clusterName, limit, offset })}";
        return await _cache.GetOrCreateAsync(
            ...
            async () =>
            {
                ...
                // Note: skillLevel and divisionId filters are not implemented because
                // these fields do not exist on the LearnCourse entity
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return await _cache.GetOrCreateAsync(
            ...
            async () =>
            {
                // Require at least one of tagId or tagName
                if (string.IsNullOrWhiteSpace(tagId) && string.IsNullOrWhiteSpace(tagName))
                {
                    _logger.LogWarning("get_courses_by_tag called without tagId or tagName");
                    return new McpCourseListResponse(
